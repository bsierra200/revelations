...
namespace BdiSearch\Service;


use Elasticsearch\Client;
use InternationalSearch\Service\EsSearcher;

class EsSearcherBdi extends EsSearcher
{
    /**
     * @var Client
     */
     ...
     * @var string
     */
    private $indexName = 'bdi_australia_individual';
            ...
            'body' => [
                "query" => [
                    "multi_match" => [
                        "query" => $fullName,
                        "operator" => "AND",
                        "fields" => ["defendant", "applicant", "company"]
                    ]
                ]
            ]
        ];
        ...
        if (count($results['hits']['hits']) == 0) {
            ...
            foreach ($results['hits']['hits'] as $processedResult) {
             //   $processedResult['_source']['state']=(is_array($processedResult['_source']['state']))?$processedResult['_source']['state'][0]:$processedResult['_source']['state'];
                ...
                $resultsProcessed['results'][$resultCounter]["index"] = $this->indexName;
                $resultCounter++;
            }
        }
